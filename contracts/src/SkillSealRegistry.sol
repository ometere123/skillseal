...
contract SkillSealRegistry is AccessControl, ReentrancyGuard {
    ...
    function withdrawBond() external nonReentrant { Bond storage bond = bonds[msg.sender]; uint128 amount = bond.pending; if (amount == 0 || block.timestamp < bond.availableAt) revert InvalidState(); bond.pending = 0; bond.availableAt = 0; (bool ok,) = msg.sender.call{value: amount}(""); if (!ok) revert TransferFailed(); emit BondWithdrawn(msg.sender, amount); }
}
